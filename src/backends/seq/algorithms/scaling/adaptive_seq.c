      ...
      if (local_nnz > 5) { 
        ...
        for (matgen_index_t dst_row = y0; dst_row <= y1; dst_row++) {
          for (matgen_index_t dst_col = x0; dst_col <= x1; dst_col++) {
            ...
            // If none of the above, weight remains 0 (should not happen) 
            if (weight > 1e-12) {
        ...
        // Sparse region: Nearest neighbor interpolation
        ...
        err = matgen_triplet_buffer_add(buffer, dst_row, dst_col, src_val);
        if (err != MATGEN_SUCCESS) {
          MATGEN_LOG_ERROR("Failed to add entry to buffer at (%llu, %llu)",
                           (unsigned long long)dst_row,
                           (unsigned long long)dst_col);
